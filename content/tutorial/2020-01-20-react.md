...
* 상위 컴포넌트에서 하위 컴포넌트로 물려 받는 속성(데이터)
...
* 컴포넌트가 만들어질 때 가장 먼저 설정되는 것이기 때문에 constructor 안에 위치한다.

* sate에 있는 데이터를 변경하기 위해서는 this.setState()라는 내장 메소드를 사용해야만 변경을 인식한다.

* state is changeble

* props is unchangeble


## 6. LifeCycle


* `LifeCycle` 란 컴포넌트가 생성되고 소멸되는 모든 과정

* lifeCycle Event 함수들은 `특정`시점에 실행된다. 각각 함수마다(componentWillMount,componentDidMount,componentWillUnmount .....)마다 실행되는 시점이 다르다.  
