...
* 쉽게 설명하면 부여받은 IP가 외부에서 내 장치에 접근할 수 있도록 포트, 즉 문을 열어주는 것입니다.
  ...
  * 주로 가정의 네트워크 환경은 인터넷사업자(ISP-Internet Service Provider)가 제공한 모뎀 - 공유기 - 디바이스(PC등...)으로 구성되어 있습니다. 이러한 경우 네트워크는 주로 하나의 고유한 IP를 가지게 되며, 이는 사업자로부터 제공됩니다. (이 고유한 IP는 하나의 자원이며, 돈입니다)
이때, ISP에 할당된 IP (Public IP)는 공유기가 가지게 됩니다. 그리고 그 공유기가 공유기에 연결된 여러개의 기기에 가상의 IP (Local IP)를 할당해줍니다. 기본적으로 허가되지 않은 외부로부터의 접속을 차단하게 되며, 외부에서 고유 IP로 연결되더라도 공유기까지만 닿을 뿐 그 공유기에 접속해 있는 디바이스까지는 닿을 수 없게 됩니다.


  * 또한 가상 IP, Local IP는 내부 네트워크 안에서만 사용되므로 외부의 컴퓨터가 내부의 IP가 할당 된 디바이스에 접속할 수 없습니다.
하지만 사용자가 외부로부터의 접속을 허용하고 싶은 경우 사용하는 방법이 바로 포트 포워딩(Port Forwarding) 또는 DMZ(Demilitarized Zone) 설정입니다.
IP는 응용 프로그램들의 통신을 구분할 수 있도록 여러개의 포트(아래 표 참조)가 있는데, 포트포워딩의 경우 IP의 특정 포트를 네트워크의 내부 특정 컴퓨터로, DMZ 설정은 IP 포트 전체를 내부 네트워크의 특정 컴퓨터로 옮겨주는 방식입니다.
[출처] 포트포워딩 (Port Forwarding)?|작성자 재이아버지
