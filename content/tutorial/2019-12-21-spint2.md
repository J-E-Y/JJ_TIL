...
이진 탐색 트리는 4개의 조건이 있다
  ...
  * 1. Every node's key is unique. (모든 노드들은 유일하다. )

  * 2. The keys in the left subtree are less than the keys in the root. (왼쪽은 항상 오른쪽보다 작다.)

  * 3. The keys of the right subtree are larger than the keys of the root. (오른쪽은 항상 왼쪽보다 크다. )

  * 4. 모든 트리 노드들은  2개의 자식 노드`만` 를 가질 수 있다.
  
  * 5. 배열보다 메모리를 효율적으로 할 수 있으면서도 데이터값을 빠르게 찾을 수 있다는 점이 장점입니다. 
