...
이진 탐색 트리는 4개의 조건이 있다
  ...
  * 1. Every node's key is unique.

  * 2. The keys in the left subtree are less than the keys in the root.

  * 3. The keys of the right subtree are larger than the keys of the root.

  * 4. 모든 트리 노드들은 최대 2개의 자식 노드를 가질 수 있다.
